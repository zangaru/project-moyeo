                ...
                <div class="py-10">
                    <div class="diy_content">
                        ...
                        <div id="ongoingEvents" class="event_con">
                            <c:forEach items="${result0.ongoingEventList}" var="event">
                                <!-- 진행 중인 이벤트 목록 -->
                                <ul>
                                    <li class="li_1">
                                    ...
                                    <li class="li_3">${event.eventStartdate.substring(0, 10)} ~ ${event.eventEnddate.substring(0, 10)}</li>
                                </ul>
                            </c:forEach>
                        </div>
                        ...
                        <div class="sel-search">
                            <form action="<c:url value="/event/"/>" method="post" class="search-area">
                                <div class="sel-search">
                        ...
                        <div class="page_t">
                            <div class="page_t">
                                <!-- 페이지 번호 -->
                                <c:choose>
                                   <c:when test="${result0.pager.startPage > result0.pager.blockSize}">
                                      <a href="<c:url value="/event/"/>?pageNum=${result0.pager.prevPage}&type=${search0.type}&keyword=${search0.keyword}">
                                         [이전]
                                      </a>
                                   </c:when>
                                   <c:otherwise>
                                ...
                                <c:forEach var="i" begin="${result0.pager.startPage}" end="${result0.pager.endPage}" step="1">
                                   <c:choose>
                                      <c:when test="${result0.pager.pageNum != i}">
                                         <a href="<c:url value="/event/"/>?pageNum=${i}&type=${search0.type}&keyword=${search0.keyword}">
                                ...
                                <c:choose>
                                   <c:when test="${result0.pager.endPage != result0.pager.totalPage}">
                                      <a href="<c:url value="/event/"/>?pageNum=${result0.pager.nextPage}&type=${search0.type}&keyword=${search0.keyword}">
